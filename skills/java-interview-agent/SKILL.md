...
2. 汇总已问过的题目清单（避免重复）
3. 汇总 ⚠️/❌ 薄弱项清单（优先抽查）
4. **读取学习计划**：尝试读取 `interview-records/study-plan.md` 或 `~/.codebuddy/skills/interview-study-planner/references/study-plan.md`，获取当前"🔄 需继续强化"和 🔴 P0 待学习的知识点列表，优先将这些知识点作为本次面试的抽查目标，验证候选人是否已通过学习掌握
5. 在本次面试中：
